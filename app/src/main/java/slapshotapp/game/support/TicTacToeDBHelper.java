...
public class TicTacToeDBHelper extends SQLiteOpenHelper
{
	...
	{
		super(context, TicTacToeDBContract.DATA_BASE_NAME, 
				null, 
				TicTacToeDBContract.DATA_BASE_VERSION);
	}
	...
	public boolean ContainsMacAddress(String mac)
	{
        ...
        Cursor c = db.query(
        	    TicTacToeDBContract.BluetoothConnectionsEntry.TABLE_NAME,  // The table to query
	...
	public boolean AddConnection(String devName, String mac)
	{
    	...
    	values.put(TicTacToeDBContract.BluetoothConnectionsEntry.COLUMN_NAME_DEV_NAME, 
    			devName);
    	values.put(TicTacToeDBContract.BluetoothConnectionsEntry.COLUMN_NAME_MAC_ADDRESS, 
    			mac);
    	values.put(TicTacToeDBContract.BluetoothConnectionsEntry.COLUMN_NAME_PLAYER_NAME, 
    			DEFAULT_NAME);
	...
	public boolean IsNameDefault(String mac)
	{
        ...
        if(c.getCount() == 1){
        	...
        	try{
        		String playerName = 
        			c.getString(c.getColumnIndexOrThrow(TicTacToeDBContract.BluetoothConnectionsEntry.COLUMN_NAME_PLAYER_NAME));
	...
	public String GetDeviceName(String mac)
	{
        ...
        if(c.getCount() == 1){
        	...
        	try{
        		name = 
        			c.getString(c.getColumnIndexOrThrow(TicTacToeDBContract.BluetoothConnectionsEntry.COLUMN_NAME_DEV_NAME));
	...
	{		
    	...
    	String rowUpdateClause = TicTacToeDBContract.BluetoothConnectionsEntry.COLUMN_NAME_MAC_ADDRESS + " = ?";
    	
    	//set the arguments for the row clause
    	String arguments[] = {mac};
    	
    	//try to insert the row into the database, check for an error
    	int rowsUpdated = db.update(TicTacToeDBContract.BluetoothConnectionsEntry.TABLE_NAME, 
						values, 
						rowUpdateClause, 
						arguments);
