...
public class StartBluetoothGame extends FragmentActivity implements FragmentAlertDialog, ServiceConnection, OnItemClickListener
{
    ...
    private void enableBluetoothDiscovery()
    {
        ...
        if (_myBluetoothAdapter.getScanMode() != BluetoothAdapter.SCAN_MODE_CONNECTABLE_DISCOVERABLE){
        	...
        	Toast.makeText(getApplicationContext(), 
                	R.string.discovery_enable_toast_msg, 	  
    	...
    	switch(base.getMessageID())
    	{
	    	case BluetoothMessages.VERSION_MESSAGE_ID:
	    		...
	    		if(verMsg.GetVersion() == BluetoothService.PROTOCOL_VERSION ){
					...
					myIntent.setClassName("slapshotapp.game.tictactoe", "slapshotapp.game.tictactoe.SetUpGameBluetooth");
	    			...
	    			Toast.makeText(getApplicationContext(), 
	            			R.string.incompatible_versions_msg, 
	...
	private void cancelConnection()
	{
		...
		Toast.makeText(getApplicationContext(), 
    			R.string.cancel_connection_toast, 
    			Toast.LENGTH_SHORT).show();
            ...
            if(BluetoothDevice.ACTION_FOUND.equals(action)) {
                ...
                if(_MyDBHelper.ContainsMacAddress(device.getAddress())){
                	...
                	if(!deviceAlreadyInList){//add the device to the list
                		...
                		_previouslyPlayedAgainstDevices.insert( playerName + "'s " + devName, _previouslyPlayedDeviceCount);
	        	...
	        	if(!_AttemptingConnection){
		            ...
		            {
		            	Toast.makeText(getApplicationContext(), 
		            			R.string.no_device_toast_message, 
		            			Toast.LENGTH_LONG).show();	            	
		            }else
		            {
		            	Toast.makeText(getApplicationContext(), 
		            			R.string.discovery_complete_toast, 
		            			Toast.LENGTH_SHORT).show();	
		            }
	        	}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data)
	{
		if(requestCode == ENABLE_DISCOVERY_QUERY){
			...
			if(resultCode == RESULT_CANCELED){
				Toast.makeText(getApplicationContext(), 
	        			R.string.discovery_not_enabled_toast_msg, 
	        			Toast.LENGTH_LONG).show();
			}
